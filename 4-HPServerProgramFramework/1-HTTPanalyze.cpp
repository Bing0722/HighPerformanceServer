...
/* 为了简化问题 我们并没有给客户端发送一个完整的HTTP应答报文 
...
LINE_STATUS parse_line(char* buffer, int& checked_index, int& read_index){
    ...
    buffer中客户数据的尾部的下一个字节。buffer中第0~checked_index字节都已经分析完毕，
    ...
    for(; checked_index < read_index; ++checked_index){
        ...
        if(temp == '\r'){
            ...
            那么这次分析没有读取到一个完整的行 返回LINE_OPEN以表示还需要继续读取客户数据
            才能进行下一步分析 */
    ...
    /* 如果所以内容都分析完毕也没有遇到 "\r" 字符 则返回LINE_OPEN 表示还需要读取客户数据才能进一步分析 */
    return LINE_OPEN;
}
